    ...
    async jwt({ token, user }) {
      ...
      if (token.id) {
        try {
          ...
          if (userDoc) {
            const adminEmails = ["harshagrawal4256@gmail.com"];
            const isInitialAdmin = userDoc.email ? adminEmails.includes(userDoc.email.toLowerCase()) : false;
            
            token.role = isInitialAdmin ? "admin" : (userDoc.role || "user");
            token.isPaid = isInitialAdmin ? true : (userDoc.isPaid || false);
